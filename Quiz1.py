        ...
        if node not in path:
            ...
            if new_path:
                if not shortest or len(new_path) < len(shortest):
                    shortest = new_path
    return shortest  #Algoritmo de Búsqueda en Profundidad (DFS) para encontrar la ruta más corta. 

def find_shortest_route(graph, origin, destination):
    route = dfs(graph, origin, destination)
    if route:
        return " ".join(route)
    else:
        return "None"
...
print(find_shortest_route(grafo, origen, destino))